...
/**
 * Parses string into strings/struct array
 *
 * @param {string} str
...
module.exports = function (str, stripEmtpy = true) {
    ...
    while (strIndex < str.length) {
        ...
        if (letter === control.tree.open || letter === control.tree.close) {
            ...
            strStack += letter === control.tree.open ? 1 : -1; //bump or dump the strStack based on tree struct
            ...
            if (letter === control.tree.close && strStack === 0) {
                const currentStruct = this.parseStruct(currentString.substr(1, currentString.length - 1));

                strData.push(currentStruct);
    ...
    if (strStack !== 0) {
        ...
        /**
         * Remove empty entries while not in stripEmtpy mode
         */
        const dataFiltered = strData.filter(item => !stripEmtpy || item.length !== 0);
