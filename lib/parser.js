...
const traverseString = (str, fnOnControlChar, fnEvery = () => {}) => {
    let strStack = 0;

    str.split("").forEach((letter, strIndex) => {
        const isControlTreeOpen = letter === CONTROL_TREE.open;

        if (isControlTreeOpen || letter === CONTROL_TREE.close) {
            if (isControlTreeOpen) {
                strStack++;
            } else {
                strStack--;
            }

            fnOnControlChar(isControlTreeOpen, letter, strIndex, strStack);
        }

        fnEvery(letter, strIndex, strStack);
    });
...
module.exports = class extends YnaLogger {
    ...
    parseBlockData(str) {
        ...
        while (strIndex < str.length) {
            ...
            if (
                strStack === 0 &&
                (letter === CONTROL_DATA.delimiter ||
                    (letter === CONTROL_DATA.start && !encounteredDataStart))
            ) {
                const currentString = str.substr(
                    strIndexLast,
                    strIndex - strIndexLast
